...
| `M365_CONTINUATION_ON_COMPLETE` | 开启 | 疑似长度截断的自动续写：完成回复 ≥阈值 且结尾明显半句时，网关在同一云端会话中透明地续一轮，拼回完整答案（协议里 M365 被每轮输出上限截断时**没有任何标记位**，此为启发式）。续写结果过短（<240 字符且自身不像被截断）则视为模型收尾语、保留原文。不想启用设 `0`。 |
| `M365_CONTINUATION_MIN_CHARS` | `3000` | 上述启发式的长度阈值（字符）。只影响"看起来被截断"的长回复，短回复永不触发。 |
